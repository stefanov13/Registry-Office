...
from core.custom_views.extra_content_list_view import ExtraContentListView
...
class OrdersDashboardView(
    ...
    def export_data(self):
        ...
        # Write headers to the worksheet
        headers = [str(field.verbose_name) for field in AdministrativeOrdersLogModel._meta.fields]
        worksheet.append(headers)

        # Write data to the worksheet
        for obj in queryset:
            row = [str(getattr(obj, field.name)) for field in AdministrativeOrdersLogModel._meta.fields]
            worksheet.append(row)

        # Create a response with the Excel file
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = 'attachment; filename=orders_exported_data.xlsx'
        workbook.save(response)
